...
import { Shield, Plus, Edit2, Trash2, Mail, CheckCircle2, AlertCircle, KeyRound } from 'lucide-react';
import { useUserStore, Role } from '@/store/userStore';

export default function UserManager() {
  const { authorizedUsers, isLoading, fetchUsers, addUser, updateUserRole, removeUser } = useUserStore();
  ...
  const handleAddUser = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newEmail || !newEmail.includes('@') || !newPassword) return;
    
    await addUser(newEmail.trim(), newPassword.trim(), newRole);
    setNewEmail('');
    setNewPassword('');
    setNewRole('Manager');
    setIsAdding(false);
    ...
    <div className="bg-white dark:bg-dark-800 p-8 rounded-2xl border border-slate-200 dark:border-white/5 shadow-sm">
      ...
      {isAdding && (
        <div className="mb-8 p-6 bg-slate-50 dark:bg-dark-900/50 rounded-xl border border-slate-200 dark:border-slate-700">
          ...
          <form onSubmit={handleAddUser} className="flex flex-col md:flex-row gap-4 flex-wrap">
            ...
            <div className="flex gap-2 w-full md:w-auto">
              <button
                type="submit"
                className="flex-1 md:flex-none bg-primary-500 hover:bg-primary-600 text-white px-6 py-3 rounded-xl font-medium transition-colors"
              >
                Grant
              </button>
              <button
                ...
                className="flex-1 md:flex-none bg-white dark:bg-dark-800 hover:bg-slate-100 dark:hover:bg-dark-700 border border-slate-200 dark:border-slate-700 px-6 py-3 rounded-xl transition-colors"
              >
                Cancel
              </button>
            </div>
          ...
          <div className="mt-4 flex flex-col gap-1 text-sm text-slate-500">
            <div className="flex items-start gap-2">
              ...
              <p>The user will sign in using this exact Email address and Password.</p>
            </div>
          </div>
        </div>
      )}

      {isLoading && authorizedUsers.length === 0 ? (
                ...
                <tr key={user.id} className="hover:bg-slate-50 dark:hover:bg-dark-800/50 transition-colors group">
                  <td className="py-4 font-medium text-slate-900 dark:text-white flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-slate-100 dark:bg-dark-700 flex items-center justify-center text-slate-500">
                      {user.email === 'artistantor@gmail.com' ? <Shield size={14} className="text-primary-500" /> : <Mail size={14} />}
                    </div>
                    {user.email}
                    {user.email === 'artistantor@gmail.com' && (
